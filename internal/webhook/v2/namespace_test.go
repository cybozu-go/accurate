...
var _ = Describe("Namespace webhook", func() {
	...
	It("should allow creating a normal namespace", func() {
		...
		ns.Labels = map[string]string{accuratev2.LabelTemplate: "default"}
	...
	It("should allow referencing a template namespace", func() {
		...
		instance.Labels = map[string]string{
			accuratev2.LabelTemplate: "tmpl1",
		}
	...
	It("should deny creating a self-referencing namespace", func() {
		...
		ns.Labels = map[string]string{
			accuratev2.LabelParent: "self-reference",
		}
		...
		ns.Labels = map[string]string{
			...
			accuratev2.LabelTemplate: "self-reference",
		}
		Expect(k8sClient.Create(ctx, ns)).To(MatchError(ContainSubstring("circular reference is not permitted")))
	})
	...
	It("should deny creating a sub-namespace having a template", func() {
		...
		ns.Labels = map[string]string{
			accuratev2.LabelParent:   "kube-system",
			accuratev2.LabelTemplate: "default",
		}
	...
	It("should deny creating a dangling sub-namespace", func() {
		...
		ns.Labels = map[string]string{accuratev2.LabelParent: "notexist"}
	...
	It("should deny creating a sub-namespace under non-root/non-sub namespace", func() {
		...
		ns.Labels = map[string]string{accuratev2.LabelType: "not-a-root"}
		Expect(k8sClient.Create(ctx, ns)).To(Succeed())

		sub := &corev1.Namespace{}
		sub.Name = "sub-of-non-root-non-sub"
		sub.Labels = map[string]string{accuratev2.LabelParent: "non-root-non-sub"}
	...
	It("should allow creating a sub-namespace under a root namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "create-root"}
	...
	It("should allow creating a sub-namespace under another sub-namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "create-root2"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub2 := &corev1.Namespace{}
		sub2.Name = "sub2-of-create-root2"
		sub2.Labels = map[string]string{accuratev2.LabelParent: "sub-of-create-root2"}
	...
	It("should deny updating a sub-namespace that would create a circular reference", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "non-circular-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub2 := &corev1.Namespace{}
		sub2.Name = "sub2-of-non-circular-root"
		sub2.Labels = map[string]string{accuratev2.LabelParent: "sub-of-non-circular-root"}
		Eventually(k8sClient.Create(ctx, sub2)).Should(Succeed())

		sub.Labels[accuratev2.LabelParent] = "sub2-of-non-circular-root"
	...
	It("should deny updating a template namespace that would create a circular reference", func() {
		...
		sub.Labels = map[string]string{
			accuratev2.LabelType:     accuratev2.NSTypeTemplate,
			accuratev2.LabelTemplate: "non-circular-root2",
		}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
		...
		sub2.Labels = map[string]string{
			accuratev2.LabelType:     accuratev2.NSTypeTemplate,
			accuratev2.LabelTemplate: "sub-of-non-circular-root2",
		}
		Eventually(k8sClient.Create(ctx, sub2)).Should(Succeed())

		sub.Labels[accuratev2.LabelTemplate] = "sub2-of-non-circular-root2"
		Expect(k8sClient.Update(ctx, sub)).To(MatchError(ContainSubstring("circular reference is not permitted")))
	})
	...
	It("should deny updating a sub-namespace to have a template", func() {
		...
		ns.Labels = map[string]string{
			...
			accuratev2.LabelTemplate: "dusht-tmpl",
		}
		Eventually(k8sClient.Create(ctx, ns)).Should(Succeed())

		sub := &corev1.Namespace{}
		sub.Name = "sub-of-template-root"
		sub.Labels = map[string]string{accuratev2.LabelParent: "template-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub.Labels[accuratev2.LabelTemplate] = "dusht-tmpl"
	...
	It("should deny marking a sub-namespace as a root namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "root-of-sub-mark"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub.Labels[accuratev2.LabelType] = accuratev2.NSTypeRoot
	...
	It("should deny updating a namespace having children that would become a non-root and non-sub namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "root-after-non-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub2 := &corev1.Namespace{}
		sub2.Name = "sub2-of-root-after-non-root"
		sub2.Labels = map[string]string{accuratev2.LabelParent: "sub-of-root-after-non-root"}
		Eventually(k8sClient.Create(ctx, sub2)).Should(Succeed())
		...
		delete(sub.Labels, accuratev2.LabelParent)
	...
	It("should deny updating a template namespace having one or more instances that would become a non-template", func() {
		...
		child.Labels = map[string]string{accuratev2.LabelTemplate: "tmpl-to-non-tmpl"}
	...
	It("should allow turning a sub-namespace w/o children into a normal namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "root-of-depth1"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
	...
	It("should allow turning a sub-namespace w/ children into a root namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "root-for-sub-to-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub2 := &corev1.Namespace{}
		sub2.Name = "sub2-of-root-for-sub-to-root"
		sub2.Labels = map[string]string{accuratev2.LabelParent: "sub-of-root-for-sub-to-root"}
		Eventually(k8sClient.Create(ctx, sub2)).Should(Succeed())

		sub.Labels = map[string]string{accuratev2.LabelType: accuratev2.NSTypeRoot}
		Expect(k8sClient.Update(ctx, sub)).To(Succeed())
	})
	...
	It("should deny changing a sub-namespace into a dangling sub-namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "dangling-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub.Labels[accuratev2.LabelParent] = "none"
	...
	It("should deny changing an instance namespace into a dangling namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelTemplate: "dangling-root2"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub.Labels[accuratev2.LabelTemplate] = "none"
		Expect(k8sClient.Update(ctx, sub)).To(MatchError(ContainSubstring("parent namespace does not exist: none")))
	})
	...
	It("should deny moving a sub-namespace under non-root/non-sub namespace", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "move-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
		...
		sub.Labels[accuratev2.LabelParent] = "move-non-root-non-sub"
	...
	It("should deny deleting a root namespace w/ children", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "delete-root"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
	...
	It("should deny deleting a sub-namespace w/ children", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "delete-root2"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())

		sub2 := &corev1.Namespace{}
		sub2.Name = "sub2-of-delete-root2"
		sub2.Labels = map[string]string{accuratev2.LabelParent: "sub-of-delete-root2"}
		Eventually(k8sClient.Create(ctx, sub2)).Should(Succeed())
	...
	It("should deny deleting a template w/ children", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelTemplate: "delete-tmpl1"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
	...
	It("should allow deleting a sub-namespace w/o children", func() {
		...
		sub.Labels = map[string]string{accuratev2.LabelParent: "delete-root3"}
		Eventually(k8sClient.Create(ctx, sub)).Should(Succeed())
